...
namespace FortnoxAPILibrary.GeneratedTests
{
    ...
    public class InvoicePaymentTests
    {
        ...
        public void Test_InvoicePayment_CRUD()
        {
            ...
            Assert.AreEqual("2020-01-01", createdInvoicePayment.PaymentDate?.ToString(APIConstants.DateFormat));

            #endregion CREATE

            #region UPDATE

            createdInvoicePayment.PaymentDate = new DateTime(2020,2,1);

            var updatedInvoicePayment = connector.Update(createdInvoicePayment); 
            MyAssert.HasNoError(connector);
            Assert.AreEqual("2020-02-01", updatedInvoicePayment.PaymentDate?.ToString(APIConstants.DateFormat));
            ...
            MyAssert.HasNoError(connector);
            Assert.AreEqual("2020-02-01", retrievedInvoicePayment.PaymentDate?.ToString(APIConstants.DateFormat));
